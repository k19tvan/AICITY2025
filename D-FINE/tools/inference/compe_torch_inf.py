...
def process_directory_sequentially_timed(model, device, input_dir, output_json, 
                                         day_threshold_map, night_threshold_map, 
                                         default_threshold):
    """
    Processes all images in a directory sequentially with batch size 1,
    applies ToD-specific per-class thresholds, times each step, and
    outputs results in COCO JSON format.
    """
    ...
    print(f"Found {len(image_files)} images. Starting sequential processing for COCO JSON output...")
    all_detections_list = []
    total_loading_time, total_inference_time, total_postprocessing_time = 0, 0, 0
    ...
    for i, image_path in enumerate(image_files):
        ...
        # --- 1. Image Loading & Preprocessing (Timed) ---
        ...
        try:
            ...
            # Preprocessing: BGR -> RGB, Resize, ToTensor, Normalize
            img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
            img_resized = cv2.resize(img_rgb, (640, 640), interpolation=cv2.INTER_AREA)
            im_data = torch.from_numpy(img_resized).permute(2, 0, 1).float().div(255)
            im_data = im_data.unsqueeze(0).to(device) # Batch size 1
        except Exception as e:
            print(f"Error loading or preprocessing {image_name}: {e}", file=sys.stderr)
            failed_image_count += 1
            continue
        
        if torch.cuda.is_available(): torch.cuda.synchronize()
        load_end = time.time()
        total_loading_time += (load_end - load_start)
    ...
    if failed_image_count > 0:
         print(f"Warning: Failed to process {failed_image_count} images.")
    print(f"Total detections meeting thresholds: {len(all_detections_list)}")
    ...
    if elapsed_time > 0 and processed_image_count > 0:
        ...
        print("\n--- Avg Time Breakdown (per image) ---")
        print(f"Avg Image Loading & Preprocessing: {(total_loading_time / processed_image_count) * 1000:.3f} ms")
        print(f"Avg Inference: {(total_inference_time / processed_image_count) * 1000:.3f} ms")
        print(f"Avg Post-processing & JSON format: {(total_postprocessing_time / processed_image_count) * 1000:.3f} ms")
    else:
...
def main(args):
    ...
    if os.path.isdir(input_path):
        print(f"Input is a directory: {input_path}. Processing sequentially with batch size 1.")
        process_directory_sequentially_timed(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_size", type=int, default=4, help="Batch size for video processing")
    parser.add_argument("--num_workers", type=int, default=4, help="Num workers for data loading (no longer used for directory processing)")
